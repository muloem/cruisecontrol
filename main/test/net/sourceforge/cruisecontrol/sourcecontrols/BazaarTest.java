...
public class BazaarTest {
	...
	private final static String bzrLog =   "revno: 3429 [merge]\n"
	...
	public void testParseLog() throws IOException {
        ...
        assertEquals("number of modifications", 4, mods.size());
        
        Map<String, String> properties = props.getPropertiesAndReset();
        assertEquals("has changes property", "true", properties.get("hasChanges?"));
        assertEquals("has deletions property", "true", properties.get("hasDeletions?"));
        
        Modification mod = mods.get(2);
        assertEquals("revision number", "3286.2.100", mod.revision);
        assertEquals("modifier username", "Pellan Eilner", mod.userName);
        assertEquals("modifier email", "peilner@gmail.com", mod.emailAddress);
        assertEquals("files in modification ", 2, mod.files.size());
        assertEquals("filename", "importer.py", mod.files.get(0).fileName);        
        assertEquals("filename", "errant_data.txt", mod.files.get(1).fileName);
        assertEquals("folder name", "src/schooltool/export", mod.files.get(0).folderName);
        assertEquals("folder name", "src/schooltool/export/stests", mod.files.get(1).folderName);
        ...
        assertEquals("calendar time", cal.getTime().toString(), mod.modifiedTime.toString());

        assertEquals("comment", "changed validation of all id cells to use new validation methods", mod.comment.trim());
        
        mod = mods.get(3);
        assertEquals("revision action", "added", mod.files.get(1).getAction());
        assertEquals("revision action", "modified", mod.files.get(3).getAction());
        
        int totalFiles = mods.get(0).files.size() + mods.get(1).files.size() + mods.get(2).files.size() + mods.get(3).files.size();
        assertEquals("total files in modification", 27, totalFiles);
	}

	@Test 
	public void testConditionalBuildPattern() throws IOException, CruiseControlException {
		SourceControlProperties props = new SourceControlProperties();
		final List<Modification> mods = new ArrayList<Modification>();

		Bazaar.parseLog(new StringReader(bzrLog), mods, props);
		
		bzr.setProperty("test.build");
		props.putAll(bzr.getProperties());
		
        bzr.setIncludedFiles("*/no/directory/match/*");
        bzr.resetProperty(mods, props);
        
        Map<String, String> resetProps = props.getPropertiesAndReset();
        assertEquals("reset build property", "false", resetProps.get("test.build"));
	}
	...
	public void testBzrRevisionSpec() {
		...
		assertEquals("revision spec date", "date:2012-05-22,02:51:25", Bazaar.bzrRevisionSpec(cal.getTime()));
	}
	...
	public void testValidate() throws IOException {
        ...
        bzr.setLocalWorkingCopy(tempFile.getParent());
        try {
        	bzr.setIncludedFiles("*.txt,*.java");
        	bzr.validate();
            fail("should throw exception when 'includedFiles' is set without 'property' being set");
        } catch (CruiseControlException e) {
        	// expected
        }
        ...
        try {
        	...
        	fail("should not throw exception when 'includedFiles' and 'property' are set");
        }
	}
    ...
    public void testParseErrorLog() throws IOException {
        ...
        assertEquals("has changes property", null, pm.get("hasChanges?"));
        assertEquals("has deletions property", null, pm.get("hasDeletions?"));
    }
}
